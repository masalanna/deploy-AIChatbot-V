// ============================================================
// script12.js  —  Softdel Virtual Assistant frontend
// ============================================================
// Changes from original:
//  - scheduleKeywords moved to TOP (fixes ReferenceError in sendMessage)
//  - handleOptionClick "schedule call" branch fixed (formContainer scope bug)
//  - handleSubOptionClickWithDisable duplicate removed, messagex → message fixed
//  - sendMessage: data.show_form handled (triggers form from backend response)
//  - Post-schedule success: safe chatContainer/avatar handling
//  - Removed debug alert('Function working correct')
//  - All dead commented-out code blocks removed for readability
//  - Everything else (UI, form, validation, topic buttons) unchanged
// ============================================================
...
// ── Schedule keywords ────────────────────────────────────────
// MUST be declared before sendMessage() runs — moved to top.
const scheduleKeywords = [
    "schedule", "meeting", "call", "connect",
    "talk", "speak", "demo", "representative",
    "sales", "callback"
];
...
function sendMessage() {
    ...
    // FIX: scheduleKeywords now declared at top — no ReferenceError
    ...
    .then(data => {
        setTimeout(() => {
            ...
            // FIX: handle show_form from backend (scheduling intent response)
            if (data.show_form) {
                showScheduleForm();
                return;
            }
    ...
    meetingForm.addEventListener("submit", async function (event) {
        ...
        try {
            ...
            if (data.success) {
                ...
                // FIX: safe chatContainer reference + no avatar (avatar is commented out in HTML)
                setTimeout(() => {
                    clearChat();
